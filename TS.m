...
tspan = 0:Tp:Tsym;
F1 = F1out(F1in, F1in-10, Top, n, Tp);
Fd = Fdout(Fd0, n);
ymin = 0;
ymax = 120;
zbocze = 10;
l = 5; %%liczba modeli
graniczne = zbioryrozmyte(ymin, ymax, l, zbocze);
...
h2_grupa = zeros(n+1, l); %%grupowo zebrane wszystkie odpowiedzi skokowe modeli
for i = 1 : l
...
for k = 2: n+1
    ...
    y = 0;
    for i = 1 : l
        if h2_TS_pop >= graniczne(i, 1) && h2_TS_pop <= graniczne(i, 2)
            h2_w_akt(1, i) = h2_grupa(k, i);
            sila_odpalenia_w_akt(1, i) = funprzyn(h2_TS_pop, l, ymin, ymax, zbocze, i); 
            y = y + sila_odpalenia_w_akt(1, i) * h2_w_akt(1, i);
        end
    end
    
%         [t, V_lok] = model_lin(tspan(1:k), y0, F1(1:k, 1), Fd(1:k, 1), a1, a2_w(1, i), a3, b1, b2_w(1,i), Tp, tspan(k));
%         h2_w_akt(1, i) = H2zlin(V_lok(k, 2), Ch2, V2lin_w(1, i));
%         h2_w_pop(1, i) = h2_w_akt(1, i);
%         sila_odpalenia_w_akt(1, i) = funprzyn(h2_w_akt(1, i), l, ymin, ymax, zbocze); 
